...
initialDist = ['Conc1', 'Conc3', 'uni']
top = 'dendrimer'
dist1filename = 'SIS_Polya_Testing/10Nodes/polya_post_grad_10N_{top}_B12_R2_M5_{dist}.csv'.format(top=top, dist=initialDist[0])
dist2filename = 'SIS_Polya_Testing/10Nodes/polya_post_grad_10N_{top}_B12_R2_M5_{dist}.csv'.format(top=top, dist=initialDist[1])
dist3filename = 'SIS_Polya_Testing/10Nodes/polya_post_grad_10N_{top}_B12_R2_M5_{dist}.csv'.format(top=top, dist=initialDist[2])
dist1 = pd.read_csv(dist1filename, header=None).values.tolist()
dist2 = pd.read_csv(dist2filename, header=None).values.tolist()
dist3 = pd.read_csv(dist3filename, header=None).values.tolist()
sum_metrics = [4*[0] for _ in range(len(dist1))]
for i in range(len(dist1)):
    ...
    for j in range(4):
        sum_metrics[i][j] = (rowd1[j] + rowd2[j] + rowd3[j])/3
sum_metrics[0].append((dist1[0][4]+dist2[0][4]+dist3[0][4])/3)
outputFile = 'SIS_Polya_Testing/10Nodes/polya_post_grad_10N_{top}_B12_R2_M5_{dist}.csv'.format(top=top, dist='avg')
with open(outputFile, "w+") as my_csv:
    csvWriter = csv.writer(my_csv, delimiter=',')
    csvWriter.writerows(sum_metrics)            